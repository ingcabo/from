...
variable "environment" {
  description = "AWS environment target"
}
...
variable "codepipeline_branch" {
  description = "Name of the branch to fetch from the repo for CodePipeline"
}
...
variable "dns_names" {
  type        = "list"
  description = "Array with the frontend FQDNs - Used also to set the Alternate Domain Names (CNAMEs) setting on Cloudfront"
}
...
variable "cloudfront_secret_header" {
  description = "Secret value for Cloudfront restriction header in S3 bucket"
}