...
resource "aws_iam_role" "codepipeline_role" {
  name               = "${var.environment}-${module.unique_label.name}-codepipeline-role"
  assume_role_policy = "${data.aws_iam_policy_document.codepipeline_assume_policy.json}"
}

# CodePipeline policy needed to use CodeCommit and CodeBuild
resource "aws_iam_role_policy" "attach_codepipeline_policy" {
  name = "${var.environment}-${module.unique_label.name}-codepipeline-policy"
...
resource "aws_iam_role" "codebuild_assume_role" {
  name = "${var.environment}-${module.unique_label.name}-codebuild-role"
...
resource "aws_iam_policy" "custom" {
  name   = "${module.unique_label.id}${var.char_delimiter}custom"
  policy = "${var.build_custom_iam_policy}"
}
...
resource "aws_iam_role_policy" "codebuild_policy" {
  name = "${var.environment}-${module.unique_label.name}-codebuild-policy"
  role = "${aws_iam_role.codebuild_assume_role.id}"
...
resource "aws_codebuild_project" "build_project" {
  name           = "${var.environment}-${var.repo_name}-build"
...
resource "aws_codebuild_project" "test_project" {
  name           = "${var.environment}-${var.repo_name}-test"
  description    = "The CodeBuild project for ${var.repo_name}"
  service_role   = "${aws_iam_role.codebuild_assume_role.arn}"
  build_timeout  = "${var.build_timeout}"
  encryption_key = "${aws_kms_key.artifact_encryption_key.arn}"
