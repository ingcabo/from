...
resource "aws_iam_policy" "default_cache_bucket" {
  count  = "${var.enabled == "true" && var.cache_enabled == "true" ? 1 : 0}"
  name   = "${module.label.id}${var.delimiter}cache${var.delimiter}bucket"
  path   = "/service-role/"
...
resource "aws_iam_policy" "custom" {
  name   = "${module.label.id}${var.delimiter}custom"
...
resource "aws_codebuild_project" "default" {
  ...
  environment {
    ...
    environment_variable = [{
      ...
      {
        "name"  = "STAGE"
        "value" = "${signum(length(var.environment)) == 1 ? var.environment : "UNSET"}"
      },
      {
        "name"  = "GITHUB_TOKEN"
